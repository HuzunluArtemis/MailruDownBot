...
class Main(object):
	...
	def fetch_storage(self, url):
		print('Fetching storage: %s' % (url), True)
		...
		if weblink_get_len != 2:
			raise Exception(
				'Unexpected dispatcher.weblink_get count: %s' % (weblink_get_len)
			)
		weblink_get_url = weblink_get['url']
		print('Found URL: %s' % (weblink_get_url), True)

		state_id = cloud_settings['state']['id']
		print('Found ID: %s' % (state_id), True)
		# parsed = cloud_settings
		# parsed = json.dumps(parsed, indent=4, sort_keys=True)
		# with open("apar.txt", "w", encoding="utf-8") as f:
		# 	f.write(parsed)
		# return
		folders = cloud_settings['params']['serverSideFolders']
		file_info = self.search_folders(folders, state_id)
		if not file_info:
			raise Exception('Could not find file info in folders tree')

		file_name = file_info['name']
		print('Found name: %s' % (file_name), True)

		file_size = file_info['size']
		print('Found size: %s' % (file_size), True)

		file_mtime = file_info['mtime']
		print('Found mtime: %s' % (file_mtime), True)
		...
		print('Found hash: %s' % (file_hash), True)
	...
	def fetch_token(self):
		...
		print('Fetching token: %s' % (url), True)
		...
		print('Found token: %s' % (token), True)

		return token

	def search_folders(self, folders, search_id):
		props = ['mtime', 'size', 'hash']
		queue = collections.deque([folders])
		while len(queue):
			entry = queue.pop()
			if self.dict_has_props(entry, props) and entry['public']['id'] == search_id:
				return entry
	...
	def create_download_url(self, storage):
		return '%s/%s' % (
			storage['url'],
			storage['id']
		)
	...
	def create_out_dir(self):
		...
		return opt_dir if opt_dir else 'downloads'
	...
	def download(self):
		...
		url = self.create_download_url(storage)
		print('Download URL: %s' % (url), True)
		...
		print('Temporary file: %s' % (file_name_temp), True)
		...
		print('Verifying size: %s' % (storage_size), True)
		try:
		...
		if self.options.mtime:
			...
			print('Setting mtime: %s' % (storage_mtime), True)
